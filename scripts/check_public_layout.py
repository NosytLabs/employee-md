...
def main() -> None:
    ...
    with app.test_client() as client:
        ...
        assert css, 'No local styles were loaded from Flask'
    (OUTPUT / 'rendered.html').write_text(html, encoding='utf-8')
    (OUTPUT / 'rendered.css').write_text('\n'.join(css), encoding='utf-8')
    sources = []
    for name in ('web/templates/base.html', 'web/templates/index.html', 'web/static/style.css', 'web/static/tailwind.css'):
    ...
    with sync_playwright() as p:
        ...
        try:
            for width in (320, 390, 768, 1440):
                ...
                checks['code_keyboard_targets'] = page.locator('main pre').evaluate_all('(els) => els.every(el => el.tabIndex === 0)')
                ...
                results.append({'width': width, 'checks': checks, 'alignment_positions': positions, 'errors': errors})
                context.close()
        finally:
            browser.close()
